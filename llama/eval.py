...
def calculate_detailed_metrics(predictions, ground_truth, input_texts):
    ...
    for i, (pred, true) in enumerate(zip(predictions, ground_truth)):
        ...
        # Token accuracy
        curr_matches = sum(
            1 for p_tok, t_tok in zip(pred_tokens, true_tokens) if p_tok == t_tok
        )
    ...
    for i, (pred, true, inp) in enumerate(zip(predictions, ground_truth, input_texts)):
        ...
        input_length_metrics[input_len]["sequence_matches"] += (
            pred.strip() == true.strip()
        )
        ...
        output_length_metrics[output_len]["sequence_matches"] += (
            pred.strip() == true.strip()
        )
    ...
    for metrics_dict in [input_length_metrics, output_length_metrics]:
        for length in metrics_dict:
            ...
            stats["token_accuracy"] = (
                stats["token_matches"] / stats["total_tokens"]
                if stats["total_tokens"] > 0
                else 0
            )
            stats["sequence_accuracy"] = (
                stats["sequence_matches"] / stats["count"] if stats["count"] > 0 else 0
            )
...
def main():
    ...
    cleaned_input_texts = [
        re.sub(r"</?cmd>", "", item["conversations"][1]["content"])
