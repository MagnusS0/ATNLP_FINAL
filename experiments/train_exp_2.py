...
def calculate_length_accuracies(model, dataloader, device, src_vocab, tgt_vocab, use_oracle=False):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader):
            ...
            src_lengths = [sum(1 for t in seq if t.item() not in special_tokens) for seq in src]
            tgt_lengths = [sum(1 for t in seq if t.item() not in special_tokens) for seq in tgt]
            ...
            if use_oracle:
                ...
                pred = greedy_decode(
                    model, src, eos_idx, bos_idx, pad_idx, device
                )
    ...
    print(f"Overall Token Accuracy: {overall_acc:.4f}, Overall Sequence Accuracy: {overall_seq_acc:.4f}")
    
    return src_accuracies, tgt_accuracies, src_seq_accuracies, tgt_seq_accuracies, overall_acc, overall_seq_acc


def plot_length_accuracies(accuracies, title, save_path, is_token=True, color='skyblue'):
    ...
    if isinstance(next(iter(accuracies.values())), tuple):
        values = [100 * (accuracies[l][0] if is_token else accuracies[l][1]) for l in lengths]
    else:
...
def run_experiment(n_runs=5):
    ...
    test_dataloader = DataLoader(test_dataset, batch_size=hyperparams["batch_size"], shuffle=False, num_workers=4)
    ...
    for run in range(n_runs):       
        ...
        src_acc_greedy, tgt_acc_greedy, _, _, overall_acc_greedy, overall_seq_acc_greedy = calculate_length_accuracies(
            model, test_dataloader, device, test_dataset.src_vocab, test_dataset.tgt_vocab, use_oracle=False
        )
        
        print("Calculating oracle search accuracies...")
        src_acc_oracle, tgt_acc_oracle, src_seq_acc_oracle, tgt_seq_acc_oracle, overall_acc_oracle, overall_seq_acc_oracle = calculate_length_accuracies(
            model, test_dataloader, device, test_dataset.src_vocab, test_dataset.tgt_vocab, use_oracle=True
        )
    ...
    src_acc_greedy = {k: np.mean([results[f"run{i+1}"]["src_acc_greedy"][k] for i in range(n_runs)]) 
                      for k in results["run1"]["src_acc_greedy"].keys()}
    tgt_acc_greedy = {k: np.mean([results[f"run{i+1}"]["tgt_acc_greedy"][k] for i in range(n_runs)]) 
                      for k in results["run1"]["tgt_acc_greedy"].keys()}
    overall_acc_greedy = np.mean([results[f"run{i+1}"]["overall_acc_greedy"] for i in range(n_runs)])
    overall_seq_acc_greedy = np.mean([results[f"run{i+1}"]["overall_seq_acc_greedy"] for i in range(n_runs)])
    overall_std_greedy = np.std([results[f"run{i+1}"]["overall_acc_greedy"] for i in range(n_runs)])
    overall_std_seq_greedy = np.std([results[f"run{i+1}"]["overall_seq_acc_greedy"] for i in range(n_runs)])
    
    src_acc_oracle = {k: np.mean([results[f"run{i+1}"]["src_acc_oracle"][k] for i in range(n_runs)]) 
                      for k in results["run1"]["src_acc_oracle"].keys()}
    tgt_acc_oracle = {k: np.mean([results[f"run{i+1}"]["tgt_acc_oracle"][k] for i in range(n_runs)]) 
                      for k in results["run1"]["tgt_acc_oracle"].keys()}
    src_seq_acc_oracle = {k: np.mean([results[f"run{i+1}"]["src_seq_acc_oracle"][k] for i in range(n_runs)])
                            for k in results["run1"]["src_seq_acc_oracle"].keys()}
    tgt_seq_acc_oracle = {k: np.mean([results[f"run{i+1}"]["tgt_seq_acc_oracle"][k] for i in range(n_runs)])
                            for k in results["run1"]["tgt_seq_acc_oracle"].keys()}
    overall_acc_oracle = np.mean([results[f"run{i+1}"]["overall_acc_oracle"] for i in range(n_runs)])
    overall_seq_acc_oracle = np.mean([results[f"run{i+1}"]["overall_seq_acc_oracle"] for i in range(n_runs)])
    overall_std_oracle = np.std([results[f"run{i+1}"]["overall_acc_oracle"] for i in range(n_runs)])
    overall_std_seq_oracle = np.std([results[f"run{i+1}"]["overall_seq_acc_oracle"] for i in range(n_runs)])
    
    # Print mean results
    print("\nMean Results:")
    print(f"Greedy Search - Mean Token Acc: {overall_acc_greedy:.4f} ± {overall_std_greedy:.4f}, Seq Acc: {overall_seq_acc_greedy:.4f} ± {overall_std_seq_greedy:.4f}")
    print(f"Oracle Search - Mean Token Acc: {overall_acc_oracle:.4f} ± {overall_std_oracle:.4f}, Seq Acc: {overall_seq_acc_oracle:.4f} ± {overall_std_seq_oracle:.4f}")
