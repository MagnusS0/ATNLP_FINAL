...
- **`dataset_llama.py`**: Data loader specifically formatted for Llama model fine-tuning.
- **`train_llama.py`**: Script for fine-tuning Llama models on the SCAN task using the Unsloth library.
- **`llama/llama_test.py`**: Inference script for testing fine-tuned Llama models using VLLM for faster generation.
- **`llama/eval.py`**: Comprehensive evaluation script for Llama model outputs with detailed metrics.
